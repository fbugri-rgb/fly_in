...
make step                     # same window, but waits for a keypress each turn
...
per turn). Use `make step` (or `python main.py --gui --step <map>`) to
advance manually — useful when walking a reviewer through the run.
